...
def parse_date(value, arg_name):
    try:
        return dt.datetime.strptime(value, "%d-%m-%Y").date()
    except ValueError as exc:
        raise argparse.ArgumentTypeError(f"Invalid {arg_name} date '{value}'. Use DD-MM-YYYY.") from exc


def resolve_date_range(argv=None):
    parser = argparse.ArgumentParser(description="Generate exam timetable")
    parser.add_argument("--start", help="Exam start date (DD-MM-YYYY)")
    parser.add_argument("--end", help="Exam end date (DD-MM-YYYY)")
    args = parser.parse_args(argv)

    start_date = parse_date(args.start, "start") if args.start else get_user_date("Enter exam START date")
    end_date = parse_date(args.end, "end") if args.end else get_user_date("Enter exam END date")
...
def main(argv=None):
    ...
    START_DATE, END_DATE = resolve_date_range(argv)
