...
def alloc_specific(tt, busy, rm, room_busy, day, slots_to_use, f, code, typ, elec, labsd, course_usage,
                   class_prefix=None, rr_state=None):
...
def alloc(tt, busy, rm, room_busy, d, f, code, h, typ="L", elec=False, labsd=set(), ex=False,
          preferred_slots=None, course_usage=None, class_prefix=None, rr_state=None):
    ...
    if preferred_slots:
        ...
        if pref_day == d:
            ...
            if total + 1e-9 >= h:
                if alloc_specific(tt, busy, rm, room_busy, pref_day, pref_slots, f, code, typ, elec, labsd, course_usage, class_prefix=class_prefix, rr_state=rr_state):
                    return True
...
def try_allocate_chunk_from_block(tt, busy, rm, room_busy, labsd, course_usage,
                                  code, faculty, typ, need, day, slots, class_prefix=None, rr_state=None):
    n = len(slots)
    for i in range(n):
        accum = 0.0; sub = []
        for j in range(i, n):
            ...
            if accum + 1e-9 >= need:
                used = room_busy.get(day, {}).get("C004", set())
                if set(sub) & used:
                    break
                for s_ in sub:
                    if tt.at[day, s_] != "": break
                else:
                    ok = alloc_specific(tt, busy, rm, room_busy, day, sub, faculty, code, typ, False, labsd, course_usage, class_prefix=class_prefix, rr_state=rr_state)
                    if ok:
...
def assign_combined_precise_durations(tt, busy, rm, room_busy, labsd, course_usage, combined_core, rr_state=None):
        ...
        for need, typ in chunks:
            ...
            for idx, (day, slots) in enumerate(valid_blocks):
                ...
                new_slots = try_allocate_chunk_from_block(tt, busy, rm, room_busy, labsd, course_usage,
                                                          ...
                                                          class_prefix="C0", rr_state=rr_state)
...
def add_csv_legend_block(ws, csv_path, legend_title, room_prefix=None, elective_room_map=None):
    ...
    headers = ["Course Code","Course Title","L-T-P-S-C","Faculty","Semester Half","Elective","Elective Room"]
...
def generate(courses, ws, label, seed, elective_sync, room_prefix=None, elective_room_map=None, room_busy_global=None):
            ...
            for h, typ in [(L,"L"), (T,"T"), (P,"P")]:
                ...
                while h > 1e-9 and attempts < 400:
                    ...
                    if sync_name and sync_name in elective_sync:
                        ...
                        if alloc(tt, busy, rm, room_busy, pref["day"], f, code, a, typ, is_elec_flag, labsd, False, preferred_slots=(pref["day"], pref["slots"]), course_usage=course_usage, class_prefix=room_prefix, rr_state=rr_state):
                            h -= a; placed = True
                    ...
                    if not placed:
                        ...
                        for d in d_order:
                            if alloc(tt, busy, rm, room_busy, d, f, code, a, typ, is_elec_flag, labsd, False, course_usage=course_usage, class_prefix=room_prefix, rr_state=rr_state):
                                h -= a; placed = True; break
                    if not placed:

                        exists_normal_space = False
                        for dcheck in days:
                            free_blocks = free(tt, dcheck, ex=False)
                            for b in free_blocks:
                                if sum(slot_dur[s] for s in b) + 1e-9 >= a:
                                    exists_normal_space = True
                                    break
                            if exists_normal_space:
                                break
                        if not exists_normal_space:
                            for d in days:
                                if alloc(tt, busy, rm, room_busy, d, f, code, a, typ, is_elec_flag,
                                        labsd, True, course_usage=course_usage,
                                        class_prefix=room_prefix, rr_state=rr_state):
                                    h -= a
                                    placed = True
                                    break
    ...
    combined_placed = assign_combined_precise_durations(tt, busy, rm, room_busy,
                                                    labsd, course_usage,
                                                    combined_core,
                                                    rr_state=rr_state)
    ...
    ws.append([""])
    return (combined_placed + priority_placed + regular_placed)
def split(c):
...
if __name__ == "__main__":
    ...
    csea_block = generate(cAf, ws1, "CSEA I First Half", seed+0, sync_sem1, room_prefix='C1', elective_room_map=elective_room_map, room_busy_global=global_room_busy)
    ...
    cseb_block = generate(cBf, ws1, "CSEB I First Half", seed+2, sync_sem1, room_prefix='C1', elective_room_map=elective_room_map, room_busy_global=global_room_busy)
